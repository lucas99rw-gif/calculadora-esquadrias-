...
st.title("Sistema Personalizado de Esquadrias v8.3")
...
if st.button("⚡ Calcular Aluminios e Vidros", type="primary"):
    ...
    if linha_principal == "Belissima / Suprema":
        if tipologia == "Janela de Correr - 2 Folhas":
            ...
            alt_folha, alt_iv = alt_marco - 208.0, alt_marco - 187.0
            f_desconto = 146.0 if versao_linha == "Belissima 40" else 194.0
            if var_integrada == "Dupla": f_desconto = 184.0 if versao_linha == "Belissima 40" else 230.0
            larg_folha = (larg_trilho - f_desconto) / 2
            titulo_obra = f"Janela Integrada Belissima ({var_integrada})"
            itens_alum = [f"78-472 (Tubo): 1 pc de {tubo_78:.0f} mm", f"Trilhos: 2 pcs de {larg_trilho:.0f} mm", f"Marco (Altura): 2 pcs de {alt_marco:.0f} mm", f"IV014/IV015: {alt_iv:.0f} mm", f"MN015 + Persiana: {largura - 129.0:.0f} mm", f"Altura da Folha: 4 pcs de {alt_folha:.0f} mm", f"SU008 (Batente): 2 pcs de {alt_folha + 16.0:.0f} mm", f"Largura da Folha: 4 pcs de {larg_folha:.0f} mm"]
            itens_vidro = [f"Vidro Janela: 2 chapas de {larg_folha - 6.0:.0f} mm x {alt_folha - 112.0:.0f} mm"]
            ...
            tubo_78, larg_trilho, alt_marco = largura - 86.0, largura - 40.0, altura - 3.0
            alt_folha, alt_iv = alt_marco - 248.0, alt_marco - 226.0
            f_desconto = 146.0 if versao_linha == "Belissima 40" else 194.0
            if var_integrada == "Dupla": f_desconto = 184.0 if versao_linha == "Belissima 40" else 230.0
            larg_folha = (larg_trilho - f_desconto) / 2
            titulo_obra = f"Porta Integrada Belissima ({var_integrada})"
            itens_alum = [f"78-472 (Tubo): 1 pc de {tubo_78:.0f} mm", f"Trilhos: 2 pcs de {larg_trilho:.0f} mm", f"Altura/Marco: 2 pcs de {alt_marco:.0f} mm", f"IV013/IV015: {alt_iv:.0f} mm", f"MN015 + Persiana: {largura - 129.0:.0f} mm", f"Altura Folha: 4 pcs de {alt_folha:.0f} mm", f"SU008 (Batente): 2 pcs de {alt_folha + 16.0:.0f} mm", f"Largura Folha: 4 pcs de {larg_folha:.0f} mm"]
            itens_vidro = [f"Vidro Porta: 2 chapas de {larg_folha - 6.0:.0f} mm x {alt_folha - 182.0:.0f} mm"]
