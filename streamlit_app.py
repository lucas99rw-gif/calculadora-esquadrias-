...
st.title("Sistema Personalizado de Esquadrias v6.3")
...
if st.button("⚡ Calcular Lista de Corte", type="primary"):
    ...
    # --- PORTA 4 FOLHAS DE CORRER ---
        ...
        titulo_obra = f"Porta 4 Fls ({versao_linha})"
        itens_para_tela = [
    ...
    # --- JANELA SANFONADA / ITALIANA / VENEZIANA ---
        ...
        if versao_linha == "Belissima 65":
            larg_folhas_geral = (larg_trilho - 155.0) / 2
        else:
            larg_folhas_geral = (larg_trilho - 110.0) / 2
        ...
        itens_para_tela = [
            ...
            f"Largura das Folhas Gerais ({versao_linha}): 2 pcs de {larg_folhas_geral:.0f} mm",
    ...
    # --- JANELA ROMANA - 2 FOLHAS ---
    elif tipologia == "Janela Romana - 2 Folhas":
        larg_trilho = largura - 24.0
        alt_marco = altura - 3.0
        alt_folha = alt_marco - 40.0
        titulo_obra = "Janela Romana 2 Fls"
        itens_para_tela = [
            f"Trilhos RO (Largura): 2 pcs de {larg_trilho:.0f} mm",
            f"Marcos / Altura RO: 2 pcs de {alt_marco:.0f} mm",
            f"Altura das Folhas RO: 4 pcs de {alt_folha:.0f} mm",
            f"Largura da Folha RO: 4 pcs de {(larg_trilho - 95.0) / 2:.0f} mm"
        ]
    ...
    # --- JANELA ROMANA - 4 FOLHAS ---
        ...
        alt_marco = altura - 3.0
        alt_folha = alt_marco - 40.0
        titulo_obra = "Janela Romana 4 Fls"
        itens_para_tela = [
            f"Trilhos RO (Largura): 2 pcs de {larg_trilho:.0f} mm",
            f"Marcos / Altura RO: 2 pcs de {alt_marco:.0f} mm",
            f"Altura das Folhas RO: 8 pcs de {alt_folha:.0f} mm",
            f"Largura das Folhas Fixas RO: 4 pcs de {(larg_trilho - 110.0) / 4:.0f} mm",
            f"Largura das Folhas Moveis RO: 4 pcs de {((larg_trilho - 110.0) / 4) + 2.0:.0f} mm"
        ]
    ...
    # --- PORTA ROMANA - 2 FOLHAS ---
        ...
        larg_trilho = largura - 24.0
        alt_marco = altura - 3.0
        alt_folha = alt_marco - 32.0
        titulo_obra = "Porta Romana 2 Fls"
        itens_para_tela = [
            f"Trilhos RO (Largura): 2 pcs de {larg_trilho:.0f} mm",
            f"Marcos / Altura RO: 2 pcs de {alt_marco:.0f} mm",
            f"Altura das Folhas RO: 4 pcs de {alt_folha:.0f} mm",
    ...
    # --- PORTA ROMANA - 4 FOLHAS ---
        ...
        alt_marco = altura - 3.0
        alt_folha = alt_marco - 32.0
        titulo_obra = "Porta Romana 4 Fls"
        itens_para_tela = [
            f"Trilhos RO (Largura): 2 pcs de {larg_trilho:.0f} mm",
            f"Marcos / Altura RO: 2 pcs de {alt_marco:.0f} mm",
            f"Altura das Folhas RO: 8 pcs de {alt_folha:.0f} mm",
